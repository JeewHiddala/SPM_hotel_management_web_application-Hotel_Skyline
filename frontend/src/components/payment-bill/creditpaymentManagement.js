...
class creditPaymentManagement extends Component {
    ...
    constructor(props) {
        ...
        this.deleteCreditPayment = this.deleteCreditPayment.bind(this);
        this.viewCreditPayment = this.viewCreditPayment.bind(this);
        this.handlePageChange = this.handlePageChange.bind(this); 
        this.onChange = this.onChange.bind(this);
        this.retrivecreditPaymentPages = this.retrivecreditPaymentPages.bind(this); 
       

    }

    componentDidMount() {
        axios.get('http://localhost:8100/creditpayment/')
            .then(response => {
                this.setState({ creditpayment: response.data.data });
                this.setState({ creditpayment: response.data.data.docs });          //pagination
    ...
    viewCreditPayment(e, creditpaymentId) {
        this.props.history.push({
            pathname: `/viewcreditPaymentDetails/${creditpaymentId}`,
            data: `${creditpaymentId}`
        });
    }

    

    retrivecreditPaymentPages(page) {               
        ...
        axios.get('http://localhost:8100/creditpayment/', {
            ...
            .then(response => {
                this.setState({ creditpayment: response.data.data.docs });
    ...
    handlePageChange = (data) => {       
        ...
        this.retrivecreditPaymentPages(selected);
    };


    deleteCreditPayment(e, creditpaymentId) {
        console.log("Delete", creditpaymentId)
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:8100/creditpayment/${creditpaymentId}`)
                Swal.fire(
                    'Deleted!',
                    'Payment Removed.',
                    'success'
    ...
    searchHandler = (event) => {
        ...
        let searchResults = this.state.creditpayment;
                        ...
                        <div className="row justify-content-evenly">
                            ...
                            <div className="col-8 align-self-stretch">
                                ...
                                <div className="float-end">
                                    ...
                                    <div className="col-4">
                                        <h3 className="h3"><b>Credit Payment Management</b></h3>
                                    </div>
                                    ...
                                    <div className="table-responsive">
                                        <table className="table">
                                            ...
                                            <tbody>
                                                {this.state.creditpayment.length > 0 && this.state.creditpayment.map((item, index) => (
                                                    <tr key={index}>
                                                        <td>{item.billNo}</td>
                                                        ...
                                                        <td><button type="button" className="btn btn-success" onClick={e => this.viewCreditPayment(e, item._id)}>View</button></td>
                         
                                                        <td><button type="button" className="btn btn-danger" onClick={e => this.deleteCreditPayment(e, item._id)}>Delete</button></td>
                                                    </tr>
                                                ))}
                                            </tbody>
